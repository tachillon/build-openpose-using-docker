#!/bin/bash

### INSTALL PREREQUISITES

UBUNTU_VERSION="$(lsb_release -r)"

# Basic
 apt-get --assume-yes update
 apt-get --assume-yes install build-essential
# General dependencies
 apt-get --assume-yes install libatlas-base-dev libprotobuf-dev libleveldb-dev libsnappy-dev libhdf5-serial-dev protobuf-compiler
 apt-get --assume-yes install --no-install-recommends libboost-all-dev
# Remaining dependencies
 apt-get --assume-yes install libgflags-dev libgoogle-glog-dev
# LMDB is needed for Caffe training, but very likely not for inference-only
 apt-get --assume-yes install liblmdb-dev

# Python2 libs (Official Ubuntu support dropped after Ubuntu 20)
if [[ $UBUNTU_VERSION == *"14."* ]] || [[ $UBUNTU_VERSION == *"15."* ]] || [[ $UBUNTU_VERSION == *"16."* ]] || [[ $UBUNTU_VERSION == *"17."* ]] || [[ $UBUNTU_VERSION == *"18."* ]]; then
    echo "Installing Python2 libs..."
     apt-get --assume-yes install python-setuptools python-dev build-essential
    hash pip2 2> /dev/null ||  apt-get --assume-yes install python-pip
     -H python2 -m pip install pip --upgrade
    if [[ $CI == true ]]; then
         -H python2 -m pip install --upgrade "numpy<1.17" protobuf
        python2 -m pip install --user "opencv-python<4.3"
    else
         -H python2 -m pip install --upgrade "numpy<1.17" protobuf "opencv-python<4.3"
    fi
fi
# Python3 libs
echo "Installing Python3 libs..."
 apt-get --assume-yes install python3-setuptools python3-dev build-essential
hash pip3 2> /dev/null ||  apt-get --assume-yes install python3-pip
 -H python3 -m pip install pip --upgrade
if [[ $CI == true ]]; then
     -H python3 -m pip install --upgrade numpy protobuf
    python3 -m pip install --user opencv-python
else
     -H python3 -m pip install --upgrade numpy protobuf opencv-python
fi

# OpenCL Generic (Official OpenPose support dropped after Ubuntu 20)
if [[ $UBUNTU_VERSION == *"14."* ]] || [[ $UBUNTU_VERSION == *"15."* ]] || [[ $UBUNTU_VERSION == *"16."* ]] || [[ $UBUNTU_VERSION == *"17."* ]] || [[ $UBUNTU_VERSION == *"18."* ]]; then
	 apt-get --assume-yes install opencl-headers ocl-icd-opencl-dev
	 apt-get --assume-yes install libviennacl-dev
fi
# OpenCV 2.4 -> Added as option
# #  apt-get --assume-yes install libopencv-dev
